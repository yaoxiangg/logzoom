...
type Server struct {
	...
	mtx	sync.Mutex
	inputs	map[string]input.InputInstance
	outputs map[string]output.OutputInstance
...
func New(configFile string) (*Server, error) {
	...
	return &Server{
		...
		inputs:  make(map[string]input.InputInstance),
		outputs: make(map[string]output.OutputInstance),
		...
		for name, inputConfig := range inputEntry {
			for i, item := range inputConfig {
				...
				if i == 0 { //There should be only 1 input per entry
					...
					instance, err := in.InitInstance(name, item.Value.(yaml.MapSlice), s.buffers[name]);
					if err != nil {
						log.Fatalf("Failed to init %s input: %v", item.Key, err)
					}
					go func(name string, instance input.InputInstance) {
						if err := instance.Start(); err != nil {
							log.Fatalf("Error starting input %s: %v", item.Key, err)
						}
					} (name, instance)
					s.inputs[name] = instance
				}
			}
		}
	}
		...
		for name, outputConfig := range outputEntry {
			for i, item := range outputConfig {
				...
				if i == 0 { //There should be only 1 output per entry
					...
					init := false
					var instance output.OutputInstance
					for route_name, value := range s.routes {
						if value.Output == name {
							instance, err = out.InitInstance(name, item.Value.(yaml.MapSlice), s.buffers[value.Input], s.routes[route_name]);
							if err != nil {
								log.Fatalf("Failed to init %s input: %v", item.Key, err)
							}
							init = true
					...
					if init == false {
						instance, err = out.InitInstance(name, item.Value.(yaml.MapSlice), nil, route.Route{Input: "", Output: "", Fields: make(map[string]string)});
						if err != nil {
							log.Fatalf("Failed to init %s input: %v", item.Key, err)
						}
					}
					go func(name string, instance output.OutputInstance) {
						if err := instance.Start(); err != nil {
							log.Fatalf("Error starting input %s: %v", item.Key, err)
						}
					} (name, instance)
					s.outputs[name] = instance
				}
			}
		}
	}
